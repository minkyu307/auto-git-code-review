...
let lastGitLabError = null;
// Helper function for making GitLab API requests
async function makeGitLabRequest(endpoint) {
...
function extractProjectPathFromUrl(webUrl) {
    if (!webUrl)
        return null;
    try {
...
function formatMergeRequest(mr) {
    ...
    const projectPath = mr.project?.path_with_namespace ||
...
async function resolveProjectId(projectIdentifier) {
    ...
    const proj = await makeGitLabRequest(`/projects/${encoded}`);
    return proj?.id ?? null;
}
...
server.tool('get-assigned-merge-requests', '현재 내 계정에 할당된 Merge Request 목록을 조회합니다', {
    state: z
    ...
    // 먼저 현재 사용자 정보를 가져옴
    const currentUser = await makeGitLabRequest('/user');
    if (!currentUser) {
        return {
            content: [
                {
                    type: 'text',
                    text: `GitLab API 인증에 실패했습니다. 토큰/네트워크/TLS 설정을 확인해주세요.\n세부: ${lastGitLabError || '원인 미상'}`,
                },
            ],
        };
    }
    ...
    const mergeRequests = await makeGitLabRequest(endpoint);
    if (!mergeRequests) {
        return {
            content: [
                {
    ...
    const mrText = `${currentUser.name} (@${currentUser.username})님에게 할당된 Merge Request (${state}):\n\n${formattedMRs.join('\n')}`;
    return {
        content: [
            {
                type: 'text',
...
server.tool('get-merge-request-changes', '특정 프로젝트의 특정 MR(iid/!번호)의 변경 파일과 diff만 반환합니다', {
    project: z
        .string()
        .describe('프로젝트 식별자. 숫자(Project ID) 또는 path_with_namespace (예: group/subgroup/project)'),
    ...
    const mrChanges = await makeGitLabRequest(`/projects/${projectId}/merge_requests/${iid}/changes`);
